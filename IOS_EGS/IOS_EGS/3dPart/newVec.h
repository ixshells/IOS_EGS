...
#include <cmath>
#include "newStaticAssert.h"

#define NEW_VEC_CLAMP(n, low, high) do{\
if(n < low) n = low;\
	...
	public:
		...
		Vec() { newStaticAssert(DIM > 0 && DIM <= 4);}
		Vec(const Vec& v) { VecAlgorithmHelp<Vec<Type, DIM>, DIM>::_assign(*this, v); }
		Vec(Type x)
		{
			newStaticAssert(DIM == 1); //DIM should be 1
			m_data[0] = x;
		}

		Vec(Type x, Type y)
		{
			newStaticAssert(DIM == 2); //DIM should be 2
			m_data[0] = x;
			m_data[1] = y;
		}
		...
		{
			newStaticAssert(DIM == 3); //DIM should be 3
			m_data[0] = x;
			m_data[1] = y;
			m_data[2] = z;
		}
		...
		{
			newStaticAssert(DIM == 4); //DIM should be 4
			m_data[0] = x;
			m_data[1] = y;
			m_data[2] = z;
			m_data[3] = w;
		...
		inline Type& x() { newStaticAssert(DIM >= 1); return m_data[0]; }
		inline Type& y() { newStaticAssert(DIM >= 2); return m_data[1]; }
		inline Type& z() { newStaticAssert(DIM >= 3); return m_data[2]; }
		inline Type& w() { newStaticAssert(DIM >= 4); return m_data[3]; }
		inline Type& r() { newStaticAssert(DIM >= 1); return m_data[0]; }
		inline Type& g() { newStaticAssert(DIM >= 2); return m_data[1]; }
		inline Type& b() { newStaticAssert(DIM >= 3); return m_data[2]; }
		inline Type& a() { newStaticAssert(DIM >= 4); return m_data[3]; }

		inline const Type& x() const { newStaticAssert(DIM >= 1); return m_data[0]; }
		inline const Type& y() const { newStaticAssert(DIM >= 2); return m_data[1]; }
		inline const Type& z() const { newStaticAssert(DIM >= 3); return m_data[2]; }
		inline const Type& w() const { newStaticAssert(DIM >= 4); return m_data[3]; }
		inline const Type& r() const { newStaticAssert(DIM >= 1); return m_data[0]; }
		inline const Type& g() const { newStaticAssert(DIM >= 2); return m_data[1]; }
		inline const Type& b() const { newStaticAssert(DIM >= 3); return m_data[2]; }
		inline const Type& a() const { newStaticAssert(DIM >= 4); return m_data[3]; }
